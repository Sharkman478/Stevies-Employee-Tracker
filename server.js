...
//   connection.query(
//     'SELECT employee_id, employee.first_name, employee.last_name, roles.title, department_name AS department, roles.salary, CONCAT(employee.first_name, " ", employee.last_name) AS employee FROM employee LEFT JOIN roles ON employee.roles_id = roles.id LEFT JOIN department ON roles.department_id = department.id LEFT JOIN employee employee ON employee.employee_id = employee.id',
//     (err, res) => {
//       if (err) throw err;
//       console.table(res);
//       start();
